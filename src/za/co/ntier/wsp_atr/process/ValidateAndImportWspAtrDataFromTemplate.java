...
public class ValidateAndImportWspAtrDataFromTemplate extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		logAttachmentStorageConfig("PROCESS START");
		int totalErrors = 0;
	...
	private void attachErrorWorkbook(X_ZZ_WSP_ATR_Submitted submitted, Workbook wb, String fileName) throws Exception {
		...
		try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
			...
			log.warning("Generated workbook bytes=" + data.length);
		...
		try {
			...
			MAttachmentEntry[] existingEntries = att.getEntries();
			for (int i = existingEntries.length - 1; i >= 0; i--) {
				MAttachmentEntry existing = existingEntries[i];
				if (existing != null && !Util.isEmpty(existing.getName(), true)
						&& existing.getName().toUpperCase().startsWith("ERROR")) {
					att.deleteEntry(i);
				}
			}
			...
			// IMPORTANT: re-read after commit using a fresh attachment object
			validateSavedAttachment(submitted, fileName, data);
	...
	private void validateSavedAttachment(X_ZZ_WSP_ATR_Submitted submitted, String fileName, byte[] originalData) throws Exception {
		logHeap("validateSavedAttachment - start");

		MAttachment savedAtt = MAttachment.get(
				Env.getCtx(),
				X_ZZ_WSP_ATR_Submitted.Table_ID,
				submitted.get_ID()
		);
		...
		if (savedAtt == null || savedAtt.getEntryCount() <= 0) {
			throw new AdempiereException("Attachment was saved, but could not be reloaded.");
		}

		MAttachmentEntry matched = null;
		MAttachmentEntry[] entries = savedAtt.getEntries();
		...
		if (matched == null) {
			throw new AdempiereException("Saved error attachment could not be found again: " + fileName);
		}

		byte[] savedBytes;
		try (InputStream is = matched.getInputStream()) {
			if (is == null) {
				throw new AdempiereException("Saved attachment stream is null for: " + fileName);
			}
			savedBytes = org.apache.commons.io.IOUtils.toByteArray(is);
		}

		log.warning("Original generated bytes=" + originalData.length + ", reloaded bytes=" + savedBytes.length);
		log.warning("Generated SHA-256=" + sha256(originalData));
		log.warning("Reloaded  SHA-256=" + sha256(savedBytes));

		if (savedBytes == null || savedBytes.length == 0) {
			throw new AdempiereException("Reloaded attachment is empty after save: " + fileName);
		}

		// Strong byte-for-byte check
		if (savedBytes.length != originalData.length) {
			throw new AdempiereException(
					"Attachment bytes changed after save. Generated=" + originalData.length
					+ ", Reloaded=" + savedBytes.length
					+ ". File=" + fileName
			);
		}

		for (int i = 0; i < originalData.length; i++) {
			if (originalData[i] != savedBytes[i]) {
				throw new AdempiereException(
						"Attachment bytes differ after save at byte position " + i
						+ ". File=" + fileName
				);
			}
		}

		// ZIP sanity check for xlsx/xlsm
		if (looksLikeZip(savedBytes)) {
			checkZipReadable(savedBytes, fileName);
		}

		log.warning("Saved attachment validation passed for: " + fileName);
		logHeap("validateSavedAttachment - end");
	}
	
	
	private void checkZipReadable(byte[] data, String fileName) throws Exception {
		try (ZipInputStream zis = new ZipInputStream(new ByteArrayInputStream(data))) {
			...
			if (count == 0) {
				throw new AdempiereException("Saved ZIP workbook has no entries: " + fileName);
			}
		} catch (Exception ex) {
			throw new AdempiereException("Saved attachment ZIP structure is corrupted: " + fileName, ex);
		}
	}
	...
	private WorkbookLoadResult loadWorkbook(X_ZZ_WSP_ATR_Submitted submitted) throws Exception {
		...
		log.warning("WSPATR-DEBUG | loadWorkbook(validate) | submittedId=" + submitted.getZZ_WSP_ATR_Submitted_ID()
				+ " | host=" + safeHostname()
		...
		for (MAttachmentEntry entry : entries) {
			...
			log.warning("WSPATR-DEBUG | loadWorkbook(validate) | entryName=" + name
		...
		try (InputStream is = selectedEntry.getInputStream()) {
			...
			if (bytes.length == 0) {
				logAttachmentStorageConfig("EMPTY ATTACHMENT DETECTED (validate stage)");
				throw new AdempiereException(
	 ...
	 * read it back for validation/import.
	 */
