...
public class ValidateAndImportWspAtrDataFromTemplate extends SvrProcess {
	...
	private void attachErrorWorkbook(X_ZZ_WSP_ATR_Submitted submitted, Workbook wb, String fileName) throws Exception {
		...
		// Validate generated bytes before attachment
	...
	private void validateSavedAttachment(X_ZZ_WSP_ATR_Submitted submitted, String fileName, byte[] originalData) throws Exception {
		...
		// POI reopen check after save
		try (Workbook reloadedWb = WorkbookFactory.create(new ByteArrayInputStream(savedBytes))) {
			if (reloadedWb.getNumberOfSheets() <= 0) {
				throw new AdempiereException("Reloaded saved workbook is invalid: no sheets found. File=" + fileName);
			}
		}
