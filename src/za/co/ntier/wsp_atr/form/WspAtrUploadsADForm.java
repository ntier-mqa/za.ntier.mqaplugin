...
public class WspAtrUploadsADForm extends ADForm implements EventListener<Event> {
    ...
    private void addRow(int submittedId, String orgName, Timestamp submittedDate, String status) {
        ...
        org.adempiere.webui.component.Button btnPrint = new org.adempiere.webui.component.Button("Print Report");
        btnPrint.setSclass("btn btn-sm btn-primary wsp-edit-purple");
        btnPrint.addEventListener(Events.ON_CLICK, (EventListener<Event>) e ->
            Events.postEvent(new Event("onPrintReport", this, Integer.valueOf(submittedId)))
        );
        v.appendChild(btnPrint);

        // 2) Upload WSP-ATR Report
        org.adempiere.webui.component.Button btnUploadReport = new org.adempiere.webui.component.Button("Upload WSP-ATR Report");
        prepareUploadButton(btnUploadReport, submittedId, UPLOADTYPE_WSP_ATR_REPORT);
        v.appendChild(btnUploadReport);

        // 3) Upload Signed Minutes
        org.adempiere.webui.component.Button btnUploadMinutes = new org.adempiere.webui.component.Button("Upload Signed Minutes");
        prepareUploadButton(btnUploadMinutes, submittedId, UPLOADTYPE_SIGNED_MINUTES);
        v.appendChild(btnUploadMinutes);

        // 4) Upload Attendance Register
        org.adempiere.webui.component.Button btnUploadAttendance = new org.adempiere.webui.component.Button("Upload Attendance Register");
        prepareUploadButton(btnUploadAttendance, submittedId, UPLOADTYPE_ATTENDANCE_REGISTER);
        v.appendChild(btnUploadAttendance);
    ...
    private void doUpload(int submittedId, String uploadType, Media media) throws Exception {
        ...
        try {
            ...
            // Create upload row
            X_ZZ_WSP_ATR_Uploads up = new X_ZZ_WSP_ATR_Uploads(Env.getCtx(), 0, trxName);
            up.setZZ_WSP_ATR_Submitted_ID(submittedId);
            up.setZZ_WSP_ATR_Upload_Type(uploadType);

            String niceType = uploadType;
            up.setName(niceType + " - " + filename + " - " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
            up.saveEx();

            // Attach file to the upload row
