...
import za.co.ntier.wsp_atr.models.X_ZZ_WSP_ATR_Non_Emp_AET_Burs_Rep;
import za.co.ntier.wsp_atr.models.X_ZZ_WSP_ATR_Report;
import za.ntier.models.MZZWSPATRSubmitted;

/**
 * Section 6.1 - Non-Employees AET and Bursaries
 *
 * Output: ZZ_WSP_ATR_Non_Emp_AET_Burs_Rep
 ...
 * - Total Planned 2025     => ZZ_Total_Planned (zz_total_planned)
 *
 * Notes:
 * - Uses FULL JOIN of done vs planned aggregates so planned-only rows still appear.
 * - ZZ_Report_Section set to "6.1".
 * - Re-run behavior: delete existing rows for report+section in output table.
 */
public class NonEmployeesAetBursariesSection61Builder extends AbstractReportSectionBuilder {

    private static final String SECTION = "6.1";
    private static final String TARGET_TABLE = "ZZ_WSP_ATR_Non_Emp_AET_Burs_Rep";
    ...
    public ReportBuildResult build(X_ZZ_WSP_ATR_Report report, MZZWSPATRSubmitted submitted,boolean consolidatedSubmission,
        ...
        final String sql =
            ...
            + "  FROM " + INPUT_TABLE + " t \n"
            + "  INNER JOIN ZZ_Learning_Programme_Ref lp \n"
            + "    ON lp.ZZ_Learning_Programme_Ref_ID = t.zz_learning_programme_type_done_id \n"
            ...
            + "    AND TRIM(COALESCE(lp.name, '')) IN ('Adult_Education_and_Training', 'Bursary') \n"
            ...
            + "  FROM " + INPUT_TABLE + " t \n"
            + "  INNER JOIN ZZ_Learning_Programme_Ref lp \n"
            + "    ON lp.ZZ_Learning_Programme_Ref_ID = t.zz_learning_programme_type_planned_id \n"
        ...
        try (PreparedStatement pstmt = DB.prepareStatement(sql, trxName)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    X_ZZ_WSP_ATR_Non_Emp_AET_Burs_Rep row =
                            new X_ZZ_WSP_ATR_Non_Emp_AET_Burs_Rep(report.getCtx(), 0, trxName);
