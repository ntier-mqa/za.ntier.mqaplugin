...
public class WorkforceEmpSummarySection21Builder extends AbstractReportSectionBuilder {
    ...
    public ReportBuildResult build(X_ZZ_WSP_ATR_Report report, MZZWSPATRSubmitted submitted,boolean consolidatedSubmission,
        ...
        final String sql =
              ...
              + "  SUM(CASE WHEN bd2.race_txt ILIKE '%afric%' AND LOWER(TRIM(bd2.gender_txt)) = 'male'   THEN 1 ELSE 0 END) AS african_male_cnt, \n"
              + "  SUM(CASE WHEN bd2.race_txt ILIKE '%afric%' AND LOWER(TRIM(bd2.gender_txt)) = 'female' THEN 1 ELSE 0 END) AS african_female_cnt, \n"
              + "  SUM(CASE WHEN bd2.race_txt ILIKE '%colour%' AND LOWER(TRIM(bd2.gender_txt)) = 'male'   THEN 1 ELSE 0 END) AS coloured_male_cnt, \n"
              + "  SUM(CASE WHEN bd2.race_txt ILIKE '%colour%' AND LOWER(TRIM(bd2.gender_txt)) = 'female' THEN 1 ELSE 0 END) AS coloured_female_cnt, \n"
              + "  SUM(CASE WHEN bd2.race_txt ILIKE '%indian%' AND LOWER(TRIM(bd2.gender_txt)) = 'male'   THEN 1 ELSE 0 END) AS indian_male_cnt, \n"
              + "  SUM(CASE WHEN bd2.race_txt ILIKE '%indian%' AND LOWER(TRIM(bd2.gender_txt)) = 'female' THEN 1 ELSE 0 END) AS indian_female_cnt, \n"
              + "  SUM(CASE WHEN bd2.race_txt ILIKE '%white%'  AND LOWER(TRIM(bd2.gender_txt)) = 'male'   THEN 1 ELSE 0 END) AS white_male_cnt, \n"
              + "  SUM(CASE WHEN bd2.race_txt ILIKE '%white%'  AND LOWER(TRIM(bd2.gender_txt)) = 'female' THEN 1 ELSE 0 END) AS white_female_cnt, \n"
              ...
              + "  SUM(CASE WHEN LOWER(TRIM(bd2.gender_txt)) = 'male'   THEN 1 ELSE 0 END) AS total_male_cnt, \n"
              + "  SUM(CASE WHEN LOWER(TRIM(bd2.gender_txt)) = 'female' THEN 1 ELSE 0 END) AS total_female_cnt \n"
              + "FROM bd2 \n"
