...
/**
 * Phase 1b/Misc (see "Phase 1b - Misc Family - Mapping.txt"): migrates mssdr_rejectionreasons into
 * SDR_RejectionReasons (a single source row). Same shape as {@link MigrateSDRQueryReasonsTable} -
 * SDR_CRMLetterGrantType_ID is UNMAPPED (constant value 1, carried as a plain integer).
 */
@Process(name = "za.co.ntier.sdr.process.MigrateSDRRejectionReasonsTable")
public class MigrateSDRRejectionReasonsTable extends SvrProcess {
    ...
    protected String doIt() throws Exception {
        ...
        if (table == null) {
            throw new IllegalStateException(TABLE_NAME + " does not exist - run AddSDRRejectionReasonsTable first");
        }
        ...
        String sql = "SELECT r.* FROM mssdr_rejectionreasons r "
                + "WHERE NOT EXISTS (SELECT 1 FROM sdr_rejectionreasons s WHERE s.id = r.id) "
                + "ORDER BY r.id" + (maxRows > 0 ? " LIMIT " + maxRows : "");

        int processed = 0;
        int created = 0;
        String readTrxName = Trx.createTrxName("SDRRejectionReasonsRead");
        ...
        writeErrorLogIfAny("migrate-sdr-rejectionreasons-errors");

        return "Processed " + processed + " mssdr_rejectionreasons row(s): " + created + " "
                + "SDR_RejectionReasons created, " + errors.size() + " error(s).";
    }
    ...
    private void processOneRow(MTable table, ResultSet rs, Map<Integer, Integer> financialYearCrosswalk)
        ...
        String trxName = Trx.createTrxName("SDRRejectionReasonsMigrate");
        ...
        try {
            ...
            if (created != null || updated != null) {
                SDRMigrationSupport.stampCreatedUpdated("sdr_rejectionreasons", "sdr_rejectionreasons_id", newId,
    ...
    private void logError(int sourceId, Exception e) {
        if (errors.size() < MAX_LOGGED_ERRORS) {
            errors.add("mssdr_rejectionreasons.id=" + sourceId + ": " + e.getMessage());
        }
    }
