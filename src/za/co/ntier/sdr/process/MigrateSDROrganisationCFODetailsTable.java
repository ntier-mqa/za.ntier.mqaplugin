...
/**
 * Phase 3 (see "Phase 3 - Organisation Family - Mapping.txt"): migrates mssdr_organisationcfodetails
 * into SDR_OrganisationCFODetails (1,281 source rows). Requires {@link MigrateSDROrganisationTable} to
 * have already run.
 */
@Process(name = "za.co.ntier.sdr.process.MigrateSDROrganisationCFODetailsTable")
public class MigrateSDROrganisationCFODetailsTable extends SvrProcess {
    ...
    protected String doIt() throws Exception {
        ...
        if (table == null) {
            throw new IllegalStateException(
                    TABLE_NAME + " does not exist - run AddSDROrganisationCFODetailsTable first");
        }

        Map<Integer, Integer> orgCrosswalk = SDRMigrationSupport.buildIdCrosswalk("sdr_organisation",
                "sdr_organisation_id", get_TrxName());

        String sql = "SELECT c.* FROM mssdr_organisationcfodetails c "
                + "WHERE NOT EXISTS (SELECT 1 FROM sdr_organisationcfodetails s WHERE s.id = c.id) "
                + "ORDER BY c.id" + (maxRows > 0 ? " LIMIT " + maxRows : "");

        int processed = 0;
        int created = 0;
        int skippedNoOrg = 0;
        String readTrxName = Trx.createTrxName("SDROrgCFODetailsRead");
        ...
        writeErrorLogIfAny("migrate-sdr-organisationcfodetails-errors");

        return "Processed " + processed + " mssdr_organisationcfodetails row(s): " + created + " "
                + "SDR_OrganisationCFODetails created, " + skippedNoOrg + " skipped (no matching "
    ...
    private void processOneRow(MTable table, ResultSet rs, int orgId) throws Exception {
        ...
        String trxName = Trx.createTrxName("SDROrgCFODetailsMigrate");
        ...
        try {
            ...
            if (created != null || updated != null) {
                SDRMigrationSupport.stampCreatedUpdated("sdr_organisationcfodetails",
                        "sdr_organisationcfodetails_id", newId, created, updated, trxName);
            }
    ...
    private void logError(int sourceId, Exception e) {
        if (errors.size() < MAX_LOGGED_ERRORS) {
            errors.add("mssdr_organisationcfodetails.id=" + sourceId + ": " + e.getMessage());
        }
    }
