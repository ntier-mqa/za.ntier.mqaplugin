...
/**
 * Phase 7 (see "Phase 7 - Grant Family - Mapping.txt"): migrates mssdr_grantaccount into
 * SDR_GrantAccount (415 source rows) - a catalog table. SDR_GrantCode is kept as plain text only (not
 * resolved to a FK - a fuzzy match against SDR_GrantCode only reaches 86.0%, per user decision).
 */
@Process(name = "za.co.ntier.sdr.process.MigrateSDRGrantAccountTable")
public class MigrateSDRGrantAccountTable extends SvrProcess {
    ...
    protected String doIt() throws Exception {
        ...
        if (table == null) {
            throw new IllegalStateException(TABLE_NAME + " does not exist - run AddSDRGrantAccountTable first");
        }
        ...
        String sql = "SELECT a.* FROM mssdr_grantaccount a "
                + "WHERE NOT EXISTS (SELECT 1 FROM sdr_grantaccount s WHERE s.id = a.id) "
                + "ORDER BY a.id" + (maxRows > 0 ? " LIMIT " + maxRows : "");

        int processed = 0;
        int created = 0;
        String readTrxName = Trx.createTrxName("SDRGrantAccountRead");
        ...
        writeErrorLogIfAny("migrate-sdr-grantaccount-errors");

        return "Processed " + processed + " mssdr_grantaccount row(s): " + created + " SDR_GrantAccount "
    ...
    private void processOneRow(MTable table, ResultSet rs, Map<Integer, Integer> financialYearCrosswalk)
        ...
        String trxName = Trx.createTrxName("SDRGrantAccountMigrate");
        ...
        try {
            ...
            if (created != null || updated != null) {
                SDRMigrationSupport.stampCreatedUpdated("sdr_grantaccount", "sdr_grantaccount_id", newId, created,
                        updated, trxName);
            }
    ...
    private void logError(int sourceId, Exception e) {
        if (errors.size() < MAX_LOGGED_ERRORS) {
            errors.add("mssdr_grantaccount.id=" + sourceId + ": " + e.getMessage());
        }
    }
