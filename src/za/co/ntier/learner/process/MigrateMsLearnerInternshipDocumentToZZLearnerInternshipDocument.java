 ...
 * ms_learnerinternshipdocument table (1,559 rows) into ZZLearnerInternshipDocument.
 *
 * <p>Set-based SQL, same architecture as {@link MigrateMsLearnerInternshipToZZLearnerInternship}.
 * ZZLearnerInternship_ID resolves via a direct join on the parent's own "id" recon column
 * (requires the parent Migrate* to have already run).
 */
@Process(name = "za.co.ntier.learner.process.MigrateMsLearnerInternshipDocumentToZZLearnerInternshipDocument")
public class MigrateMsLearnerInternshipDocumentToZZLearnerInternshipDocument extends SvrProcess {
    ...
    private static final String SOURCE_TABLE = "ms_learnerinternshipdocument";
    private static final String TARGET_TABLE = "zzlearnerinternshipdocument";
    private static final String SEQUENCE_NAME = "ZZLearnerInternshipDocument";
    ...
    protected String doIt() throws Exception {
        ...
        DB.executeUpdateEx("DROP TABLE IF EXISTS tmp_lid_source", null, trxName);
        DB.executeUpdateEx("DROP TABLE IF EXISTS tmp_lid_ms_user_xwalk", null, trxName);

        String sourceSql = "CREATE TEMP TABLE tmp_lid_source AS "
        ...
        int rowCount = DB.getSQLValueEx(trxName, "SELECT count(*) FROM tmp_lid_source");
        if (rowCount == 0) {
            DB.executeUpdateEx("DROP TABLE tmp_lid_source", null, trxName);
            return "No new " + SOURCE_TABLE + " rows to migrate.";
        }
        addLog("Migrating " + rowCount + " row(s)...");

        DB.executeUpdateEx(
                "CREATE TEMP TABLE tmp_lid_ms_user_xwalk AS "
        ...
        String insertSql =
                ...
                + " (zzlearnerinternshipdocument_id, ad_client_id, ad_org_id, created, createdby, "
                + "  updated, updatedby, isactive, zzlearnerinternshipdocument_uu, "
                + "  zzlearnerinternship_id, original_file_name, saved_file_name, file_path, "
                ...
                + "  li.zzlearnerinternship_id, src.originalfilename, src.savedfilename, src.filepath, "
                + "  uploadedbyx.ad_user_id, updatedbyx.ad_user_id, src.id "
                + "FROM tmp_lid_source src "
                + "LEFT JOIN zzlearnerinternship li ON li.id = src.learnerinternshipid "
                + "LEFT JOIN tmp_lid_ms_user_xwalk uploadedbyx ON uploadedbyx.ms_user_id = src.createdby "
                + "LEFT JOIN tmp_lid_ms_user_xwalk updatedbyx ON updatedbyx.ms_user_id = src.updatedby";

        int inserted = DB.executeUpdateEx(insertSql,
                new Object[] { adClientId, adOrgId, createdBy, createdBy }, trxName);

        DB.executeUpdateEx("DROP TABLE tmp_lid_source", null, trxName);
        DB.executeUpdateEx("DROP TABLE tmp_lid_ms_user_xwalk", null, trxName);
        ...
        try {
            ...
            if (!rs.next()) {
                throw new AdempiereException("No AD_Sequence row found for '" + SEQUENCE_NAME
                        + "' - has AddZZLearnerInternshipDocumentTable been run yet?");
            }
