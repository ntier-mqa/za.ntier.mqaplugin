...
/**
 * Phase 2 (see "Phase 2 - LearnerLearnership Family - Mapping.txt" table #4): migrates the
 * staged ms_learnerlearnershipprovider table (71,649 rows) into the brand new
 * ZZLearnerLearnershipProvider table. Same shape as
 * {@link MigrateMsLearnerLearnershipEmployerToZZLearnerLearnershipEmployer}, with ZZProvider_ID
 * (via MigrationSupport.buildIdCrosswalk against zzprovider's own "id" recon column) instead of
 * Employer_ID/BPartner.
 */
@Process(name = "za.co.ntier.learner.process.MigrateMsLearnerLearnershipProviderToZZLearnerLearnershipProvider")
public class MigrateMsLearnerLearnershipProviderToZZLearnerLearnershipProvider extends SvrProcess {
    ...
    private static final String SOURCE_TABLE = "ms_learnerlearnershipprovider";
    ...
    protected String doIt() throws Exception {
        ...
        String sql = "SELECT id, learnerlearnershipid, providerid, learnershipprovidertypeid, levyyesnoid, "
        ...
        String readTrxName = Trx.createTrxName("MsLearnerLearnershipProviderRead");
        ...
        try {
            ...
            while (rs.next()) {
                processed++;
                try {
                    processOneRow(table, rs, learnerLearnershipCrosswalk, providerCrosswalk, levyCrosswalk);
                    created++;
                } catch (Exception e) {
    ...
    private void processOneRow(MTable table, ResultSet rs, Map<Integer, Integer> learnerLearnershipCrosswalk,
            Map<Integer, Integer> providerCrosswalk, Map<Integer, String> levyCrosswalk) throws Exception {
        int sourceId = rs.getInt("id");
        Integer learnerLearnershipId = (Integer) rs.getObject("learnerlearnershipid");
        Integer providerId = (Integer) rs.getObject("providerid");
        Integer providerType = (Integer) rs.getObject("learnershipprovidertypeid");
        ...
        String trxName = Trx.createTrxName("MsLearnerLearnershipProviderMigrate");
        ...
        try {
            ...
            if (providerType != null) {
                po.set_ValueOfColumn("ZZLearnershipProviderType", providerType);
            }
            ...
            Integer targetProviderId = providerId == null ? null : providerCrosswalk.get(providerId);
            if (targetProviderId != null) {
                po.set_ValueOfColumn("ZZProvider_ID", targetProviderId);
            }
    ...
    private void writeErrorLogIfAny() {
        ...
        File logFile = new File("/tmp/migrate-ms-learnerlearnershipprovider-errors-" + ts + ".txt");
