...
/**
 * Phase 4 (see "Phase 4 - LearnerPostSchoolDetails Family - Mapping.txt"): migrates the staged
 * ms_learnerpostschooldetails table (132,139 rows) into ZZLearnerPostSchoolDetails.
 *
 * <p>Set-based SQL, same architecture as {@link MigrateLearnerDocumentsToZZLearnerDocuments} -
 * 132,139 rows exceeds the largest per-row Migrate* built so far (73,930), applying the pattern
 * now established twice (LearnerUnitStandard, LearnerDocuments) rather than re-confirming.
 *
 * <p>Only 1 real crosswalk needed: ms_learner_xref (ZZLearner_ID). ZZQualification_ID resolves
 * via a direct join on zzqualification's own "id" recon column - no temp table needed.
 * OFO_Occupation (ofoid) is carried across as its raw source value, unresolved (see class
 * Javadoc on AddZZLearnerPostSchoolDetailsTable).
 */
@Process(name = "za.co.ntier.learner.process.MigrateMsLearnerPostSchoolDetailsToZZLearnerPostSchoolDetails")
public class MigrateMsLearnerPostSchoolDetailsToZZLearnerPostSchoolDetails extends SvrProcess {
    ...
    private static final String SOURCE_TABLE = "ms_learnerpostschooldetails";
    private static final String TARGET_TABLE = "zzlearnerpostschooldetails";
    private static final String SEQUENCE_NAME = "ZZLearnerPostSchoolDetails";
    ...
    protected String doIt() throws Exception {
        ...
        DB.executeUpdateEx("DROP TABLE IF EXISTS tmp_lpsd_source", null, trxName);

        String sourceSql = "CREATE TEMP TABLE tmp_lpsd_source AS "
        ...
        int rowCount = DB.getSQLValueEx(trxName, "SELECT count(*) FROM tmp_lpsd_source");
        if (rowCount == 0) {
            DB.executeUpdateEx("DROP TABLE tmp_lpsd_source", null, trxName);
        ...
        String insertSql =
                ...
                + " (zzlearnerpostschooldetails_id, ad_client_id, ad_org_id, created, createdby, updated, "
                + "  updatedby, isactive, zzlearnerpostschooldetails_uu, zzlearner_id, zzqualification_id, "
                + "  zzdateachieved, ofo_occupation, id) "
                + "SELECT " + startId + " + src.rn - 1, ?, ?, src.created, ?, src.updated, ?, "
                + "  CASE WHEN src.isdeleted = 0 THEN 'Y' ELSE 'N' END, adempiere.generate_uuid(), "
                + "  lx.zzlearner_id, zzq.zzqualification_id, src.dateachieved, src.ofoid, src.id "
                + "FROM tmp_lpsd_source src "
                + "LEFT JOIN ms_learner_xref lx ON lx.ms_learner_id = src.learnerid "
                + "LEFT JOIN zzqualification zzq ON zzq.id = src.qualificationid";
        ...
        try {
            ...
            if (!rs.next()) {
                throw new AdempiereException("No AD_Sequence row found for '" + SEQUENCE_NAME
                        + "' - has AddZZLearnerPostSchoolDetailsTable been run yet?");
            }
