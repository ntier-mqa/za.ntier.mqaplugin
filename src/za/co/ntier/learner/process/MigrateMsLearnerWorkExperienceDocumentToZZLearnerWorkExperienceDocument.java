 ...
 * ms_learnerworkexperiencedocument table (240 rows) into ZZLearnerWorkExperienceDocument.
 *
 * <p>Set-based SQL, same architecture as {@link MigrateMsLearnerWorkExperienceToZZLearnerWorkExperience}.
 * ZZLearnerWorkExperience_ID resolves via a direct join on the parent's own "id" recon column
 * (requires the parent Migrate* to have already run).
 */
@Process(name = "za.co.ntier.learner.process.MigrateMsLearnerWorkExperienceDocumentToZZLearnerWorkExperienceDocument")
public class MigrateMsLearnerWorkExperienceDocumentToZZLearnerWorkExperienceDocument extends SvrProcess {
    ...
    private static final String SOURCE_TABLE = "ms_learnerworkexperiencedocument";
    private static final String TARGET_TABLE = "zzlearnerworkexperiencedocument";
    private static final String SEQUENCE_NAME = "ZZLearnerWorkExperienceDocument";
    ...
    protected String doIt() throws Exception {
        ...
        DB.executeUpdateEx("DROP TABLE IF EXISTS tmp_lwed_source", null, trxName);
        DB.executeUpdateEx("DROP TABLE IF EXISTS tmp_lwed_ms_user_xwalk", null, trxName);

        String sourceSql = "CREATE TEMP TABLE tmp_lwed_source AS "
        ...
        int rowCount = DB.getSQLValueEx(trxName, "SELECT count(*) FROM tmp_lwed_source");
        if (rowCount == 0) {
            DB.executeUpdateEx("DROP TABLE tmp_lwed_source", null, trxName);
            return "No new " + SOURCE_TABLE + " rows to migrate.";
        }
        addLog("Migrating " + rowCount + " row(s)...");

        DB.executeUpdateEx(
                "CREATE TEMP TABLE tmp_lwed_ms_user_xwalk AS "
        ...
        String insertSql =
                ...
                + " (zzlearnerworkexperiencedocument_id, ad_client_id, ad_org_id, created, createdby, "
                + "  updated, updatedby, isactive, zzlearnerworkexperiencedocument_uu, "
                + "  zzlearnerworkexperience_id, original_file_name, saved_file_name, file_path, "
                + "  uploaded_by, updated_by, id) "
                + "SELECT " + startId + " + src.rn - 1, ?, ?, src.created, ?, src.updated, ?, "
                + "  CASE WHEN src.isdeleted THEN 'N' ELSE 'Y' END, adempiere.generate_uuid(), "
                + "  lwe.zzlearnerworkexperience_id, src.originalfilename, src.savedfilename, src.filepath, "
                + "  uploadedbyx.ad_user_id, updatedbyx.ad_user_id, src.id "
                + "FROM tmp_lwed_source src "
                + "LEFT JOIN zzlearnerworkexperience lwe ON lwe.id = src.learnerworkexperienceid "
                + "LEFT JOIN tmp_lwed_ms_user_xwalk uploadedbyx ON uploadedbyx.ms_user_id = src.createdby "
                + "LEFT JOIN tmp_lwed_ms_user_xwalk updatedbyx ON updatedbyx.ms_user_id = src.updatedby";

        int inserted = DB.executeUpdateEx(insertSql,
                new Object[] { adClientId, adOrgId, createdBy, createdBy }, trxName);

        DB.executeUpdateEx("DROP TABLE tmp_lwed_source", null, trxName);
        DB.executeUpdateEx("DROP TABLE tmp_lwed_ms_user_xwalk", null, trxName);
        ...
        try {
            ...
            if (!rs.next()) {
                throw new AdempiereException("No AD_Sequence row found for '" + SEQUENCE_NAME
                        + "' - has AddZZLearnerWorkExperienceDocumentTable been run yet?");
            }
