...
/**
 * Phase 4 (see "Phase 4 - LearnerDocuments Family - Mapping.txt"): migrates the staged
 * learnerdocuments table (349,897 rows) into ZZLearnerDocuments.
 *
 * <p>Set-based SQL, same architecture as {@link MigrateMsLearnerUnitStandardToZZLearnerUnitStandard}
 * (user decision 2026-07-31 - this table sits between the largest per-row Migrate* built so far
 * and the smallest set-based one, and set-based was chosen for consistency/speed). See that
 * class's Javadoc for the batch-primary-key-reservation technique (unchanged here, already fixed
 * for the boxed-Long "Unknown parameter type" pitfall).
 *
 * <p>Only 2 crosswalks needed: ms_learner_xref (ZZLearner_ID) and the shared ms_user email-match
 * temp table (Uploaded_By/Updated_By - createdby/updatedby on this source are ms_user ids, not
 * raw system user ids, confirmed by range-checking against ms_user's own id range).
 *
 * <p>NOTE: unlike every "ms_"-prefixed staged table in this project, "learnerdocuments" has no
 * ad_client_id/ad_org_id columns of its own (staged in bare/raw form, same as "aet") - the
 * target's AD_Client_ID/AD_Org_ID come from this process's own running context instead. Source
 * timestamp columns are also named differently ("datecreated"/"dateupdated", not "created"/
 * "updated").
 */
@Process(name = "za.co.ntier.learner.process.MigrateLearnerDocumentsToZZLearnerDocuments")
public class MigrateLearnerDocumentsToZZLearnerDocuments extends SvrProcess {
    ...
    private static final String SOURCE_TABLE = "learnerdocuments";
    private static final String TARGET_TABLE = "zzlearnerdocuments";
    private static final String SEQUENCE_NAME = "ZZLearnerDocuments";
    ...
    protected String doIt() throws Exception {
        ...
        DB.executeUpdateEx("DROP TABLE IF EXISTS tmp_ld_source", null, trxName);
        DB.executeUpdateEx("DROP TABLE IF EXISTS tmp_ld_ms_user_xwalk", null, trxName);

        String sourceSql = "CREATE TEMP TABLE tmp_ld_source AS "
        ...
        int rowCount = DB.getSQLValueEx(trxName, "SELECT count(*) FROM tmp_ld_source");
        if (rowCount == 0) {
            DB.executeUpdateEx("DROP TABLE tmp_ld_source", null, trxName);
            return "No new " + SOURCE_TABLE + " rows to migrate.";
        }
        addLog("Migrating " + rowCount + " row(s)...");

        DB.executeUpdateEx(
                "CREATE TEMP TABLE tmp_ld_ms_user_xwalk AS "
        ...
        String insertSql =
                ...
                + " (zzlearnerdocuments_id, ad_client_id, ad_org_id, created, createdby, updated, "
                + "  updatedby, isactive, zzlearnerdocuments_uu, zzlearner_id, original_file_name, "
                + "  saved_file_name, file_path, uploaded_by, updated_by, id) "
                ...
                + "  lx.zzlearner_id, src.originalfilename, src.savedfilename, src.filepath, "
                + "  uploadedbyx.ad_user_id, updatedbyx.ad_user_id, src.id "
                + "FROM tmp_ld_source src "
                + "LEFT JOIN ms_learner_xref lx ON lx.ms_learner_id = src.learnerid "
                + "LEFT JOIN tmp_ld_ms_user_xwalk uploadedbyx ON uploadedbyx.ms_user_id = src.createdby "
                + "LEFT JOIN tmp_ld_ms_user_xwalk updatedbyx ON updatedbyx.ms_user_id = src.updatedby";

        int inserted = DB.executeUpdateEx(insertSql,
                new Object[] { adClientId, adOrgId, createdBy, createdBy }, trxName);

        DB.executeUpdateEx("DROP TABLE tmp_ld_source", null, trxName);
        DB.executeUpdateEx("DROP TABLE tmp_ld_ms_user_xwalk", null, trxName);
        ...
        try {
            ...
            if (!rs.next()) {
                throw new AdempiereException("No AD_Sequence row found for '" + SEQUENCE_NAME
                        + "' - has AddZZLearnerDocumentsTable been run yet?");
            }
